...
for stage  in [0, 1]:
    for label in keys:
        for seq in range(len(data[stage][label])):
            kmeans = KMeans(n_clusters = 5, random_state = 0).fit(data[stage][label][seq])
            data[stage][label][seq] = np.concatenate(kmeans.cluster_centers_)
        print len(data[stage][label])
print 'Convert sequence to point data'
